...
export class Generate {
	...
	static orderNumber(): string {
		...
		// Combine and ensure exactly 15 characters
		let orderNum = timestamp + random;

		// Trim or pad to ensure exactly 15 characters
		if (orderNum.length > 15) {
			orderNum = orderNum.substring(0, 15);
		} else if (orderNum.length < 15) {
			orderNum = orderNum.padEnd(15, "0");
		}
